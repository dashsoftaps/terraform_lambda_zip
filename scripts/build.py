...
def testmain(args):
    def _print_env(envvar, file=sys.stdout):
        print("os.environ['{}'] = '{}')".format(envvar, os.getenv(envvar)), file=file)

    with open("/tmp/output.txt", "w") as out:
        _print_env("OUTPUT_FILEPATH", file=out)
        _print_env("PROJECT_PATH", file=out)
        _print_env("REQUIREMENTS_FILE", file=out)
        _print_env("LIB_PATH", file=out)
